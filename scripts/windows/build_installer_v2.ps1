...
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "build_exe_v2.ps1") -RepoRoot $RepoRoot
if ($LASTEXITCODE -ne 0) {
    throw "Failed to build receiver EXE before installer packaging."
}

$exePath = Join-Path $RepoRoot "dist\STEAMDECK-MIDI-RECEIVER-2.exe"
if (-not (Test-Path $exePath)) {
    throw "Packaged receiver EXE not found at '$exePath'. Build it first with build_exe_v2.ps1."
}
...
$issPath = Join-Path $RepoRoot "installer\windows\steamdeck-midi-receiver-2.iss"
...
$setupExe = Join-Path $outputDir ("STEAMDECK-MIDI-RECEIVER-2-Setup-{0}.exe" -f $appVersion)
