...
$args='-B "' + $repeat + '" "' + $root + '" 300 test_malformed_json_and_unsupported_method_are_json ' + $expected + ' "' + $outjson + '"'
Write-Output "ROOT $root"
Write-Output "COMMAND $python $args"
$c=Start-Process -FilePath $python -ArgumentList $args -WorkingDirectory $root -NoNewWindow -PassThru -RedirectStandardOutput "$work\flake-$Which.stdout.log" -RedirectStandardError "$work\flake-$Which.stderr.log"
$null=$c.Handle; $seen=@{}
