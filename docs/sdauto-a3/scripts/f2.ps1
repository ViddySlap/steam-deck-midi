...
param([ValidateSet('head','v049')][string]$Which, [int]$Udp, [int]$Ui, [string]$Run='r1', [switch]$EnableCtrlC)
Set-StrictMode -Version Latest
$ErrorActionPreference='Stop'
$clone='C:\Users\Ben\Documents\project-workspaces\steam-deck-midi-rc'
$work=Join-Path $PSScriptRoot "f2-$Which-$Run"
...
Write-Output "BASE_PYTHON $base"
$cfg=[ordered]@{ label=$Which; python=$python; tree=$tree; map=$map; udp=$Udp; ui=$Ui; extra=$extra; browser=$browser;
  work=$work; out=(Join-Path $work 'result.json'); log=(Join-Path $work 'bridge.log'); record=(Join-Path $work "owned-tree-$Which.txt"); ctrl=(Join-Path $PSScriptRoot 'win_ctrl.py'); enable_ctrl_c=[bool]$EnableCtrlC }
...
$driver=Start-Process -FilePath $base -ArgumentList @('-B', (Join-Path $PSScriptRoot 'win_f2.py'), $cfgPath) -PassThru -NoNewWindow
$null=$driver.Handle
$did=$driver.Id
