param([string]$Label = 'gate-backfill')
...
$results = @()
$plan = @(
  @('HEAD','start_installed_receiver_v2.ps1','missing-key'),
  @('HEAD','start_receiver.ps1','absent-file'),
  @('HEAD','start_installed_receiver.ps1','absent-file'),
  @('HEAD','start_installed_receiver_v2.ps1','absent-file'),
  @('v049','start_receiver.ps1','absent-file'),
  @('v049','start_installed_receiver.ps1','absent-file'),
  @('v049','start_installed_receiver_v2.ps1','absent-file')
)
foreach ($row in $plan) {
    $ver=$row[0]; $name=$row[1]; $arm=$row[2]
    foreach ($once in @(1)) {
        $root = Join-Path $work ($ver + '-' + $name.Replace('.ps1','') + '-' + $arm)
        ...
        $launcher = if ($ver -eq 'HEAD') { Join-Path $clone ('scripts\windows\' + $name) } else { Join-Path $PSScriptRoot ('v049_' + $name) }
        ...
        $results += [ordered]@{version=$ver; launcher_path=$launcher; launcher=$name; arm=$arm; launcher_sha256=(Get-FileHash -LiteralPath $launcher -Algorithm SHA256).Hash.ToLowerInvariant(); settings_sha256_before=$beforeHash; settings_sha256_after=$afterHash; saved=$saved; calls=@($argv); exit_code=$code; powershell_pid=$child.Id; all_child_pids_gone=$true; before=$before; after=$after; receiver_started=$false; stderr=([IO.File]::ReadAllText($stderr)); stdout=([IO.File]::ReadAllText($stdout))}
        [IO.File]::WriteAllText((Join-Path $PSScriptRoot ($Label + '-arms.json')), (ConvertTo-Json -InputObject $results -Depth 8))
        if ($arm -eq 'missing-key') { Write-Output ('  saved_preset_section=' + [string]$saved.preset_section); foreach ($c in $argv) { Write-Output ('  argv=' + (ConvertTo-Json -InputObject $c.argv -Compress)) } }
        $firstErr = ([IO.File]::ReadAllText($stderr) -split "`n" | Where-Object { $_ -match 'line:|PropertyNotFound|At .*:\d+' } | Select-Object -First 3) -join ' | '
        Write-Output "  stderr_head=$firstErr"
        Write-Output "$ver $name $arm exit=$code calls=$(@($argv).Count) before=$beforeHash after=$afterHash"
    }
}
