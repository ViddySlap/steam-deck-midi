...
$exePath = Join-Path $InstallRoot "STEAMDECK-MIDI-RECEIVER-2.exe"
...
$bridgeSettingsPath = Join-Path $InstallRoot "config\bridge.local.json"
...
if ($settings.PSObject.Properties.Name -contains "no_ui" -and [bool]$settings.no_ui) {
    $args += "--no-ui"
}
if ($settings.PSObject.Properties.Name -contains "ui_port") {
    $args += "--ui-port"
    $args += [string]$settings.ui_port
}

Write-Host "STEAMDECK MIDI Receiver 2"
