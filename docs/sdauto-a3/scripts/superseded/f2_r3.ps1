...
param([ValidateSet('head','v049')][string]$Which, [int]$Udp, [int]$Ui, [string]$Run='r1')
...
$cfg=[ordered]@{ label=$Which; python=$python; tree=$tree; map=$map; udp=$Udp; ui=$Ui; extra=$extra; browser=$browser;
  work=$work; out=(Join-Path $work 'result.json'); log=(Join-Path $work 'bridge.log'); record=(Join-Path $work "owned-tree-$Which.txt"); ctrl=(Join-Path $PSScriptRoot 'win_ctrl.py') }
