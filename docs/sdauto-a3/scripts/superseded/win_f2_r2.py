...
k32.SetConsoleCtrlHandler.argtypes = [HANDLER, wt.BOOL]
...
def main():
    ...
    received = []
    signal.signal(signal.SIGINT, lambda *x: received.append("SIGINT"))
    signal.signal(signal.SIGBREAK, lambda *x: received.append("SIGBREAK"))
    result["console"] = {}
    ...
    keep_alive = HANDLER(lambda event: True)
    result["console"]["SetConsoleCtrlHandler"] = k32.SetConsoleCtrlHandler(keep_alive, True)
    result["console"]["FreeConsole"] = k32.FreeConsole()
    result["console"]["AttachConsole"] = k32.AttachConsole(proc.pid)
    result["console"]["attach_error"] = ctypes.get_last_error()
    buf = (wt.DWORD * 64)()
    n = k32.GetConsoleProcessList(buf, 64)
    attached = sorted(buf[i] for i in range(min(n, 64)))
    owned = sorted(set([os.getpid()] + list(handles)))
    result["console"]["attached_pids"] = attached
    result["console"]["private"] = bool(result["console"]["AttachConsole"]) and bool(attached) and set(attached) <= set(owned)
    save()
    ...
    for name, event in (("CTRL_C_EVENT", 0), ("CTRL_BREAK_EVENT", 1)):
        if all_gone(0):
            break
        if not result["console"]["private"]:
            stops.append({"method": name, "skipped": "console not private"})
            continue
        t = time.monotonic()
        ok = k32.GenerateConsoleCtrlEvent(event, 0)
        err = ctypes.get_last_error()
        gone = all_gone(5000)
        stops.append({"method": name, "GenerateConsoleCtrlEvent": ok, "last_error": err,
                      "tree_exited_within_5s": gone, "elapsed": round(time.monotonic() - t, 3),
